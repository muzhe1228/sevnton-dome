...
myApp.controller("BasicApplyCtl", function ($scope, $http, $rootScope, $state, $timeout) {
    $rootScope.indexID = 1;
    ...
    $scope.initPage = function () {
        $scope.$emit("checkContract", "在线下单基础服务");
    }
    $scope.declareChange = function () {
        if ($("#declareType  option:selected").val() == '0') {//圣云通报关
            $("#billOfLadingDiv").attr("style", "display:block");
            $("#voyageDiv").attr("style", "display:block");

            $("#voyage").attr("data-validate", "required:必填");
            $("#billOfLading").attr("data-validate", "required:必填");

            $("#declareTelDiv").attr("style", "display:none");
            $("#declareCompanyDiv").attr("style", "display:none");

            $("#declareCompany").removeAttr("data-validate");
            $("#declareCompany").parent("div .fields").find(".input-help").remove();
            $("#declareCompany").parent("div .fields").parent("div .form-group").removeClass("check-error");
        } else {//工厂自己报关
            $("#billOfLadingDiv").attr("style", "display:none");
            $("#voyageDiv").attr("style", "display:none");
            $("#voyage").removeAttr("data-validate");

            $("#voyage").parent("div .fields").find(".input-help").remove();
            $("#voyage").parent("div .fields").parent("div .form-group").removeClass("check-error");

            $("#billOfLading").removeAttr("data-validate");
            $("#billOfLading").parent("div .fields").find(".input-help").remove();
            $("#billOfLading").parent("div .fields").parent("div .form-group").removeClass("check-error");

            $("#declareTelDiv").attr("style", "display:block");
            $("#declareCompany").attr("data-validate", "required:必填");
            $("#declareCompanyDiv").attr("style", "display:block");
        }
    }
    ...
    function copy() {
        ...
        $http({
            ...
            if (rs.code == 0) {
                ...
                //$scope.info.currency=$scope.goods.attrs.currency_id;
    ...
    $scope.submit = function () {
        $("form").ajaxSubmit(function () {
            ...
            $http({
                ...
                layer.close(load);
                layer.msg("在线下单时发生系统异常");
            });
        });
    }
    $scope.DetialsLayer = function () {
        if ($scope.info) {
            $scope.nums = 0;
            $scope.groossWeightAll = 0;
            $scope.netWeightAll = 0;
            $scope.kinds = []
            $scope.marks = []
            angular.forEach($scope.info.goods, function (item) {
                $scope.nums += parseInt(item.realCount);
                $scope.groossWeightAll += parseInt(item.grossWeight);
                $scope.netWeightAll += parseInt(item.netWeight);
                $scope.kinds.push(item.packingType)
                $scope.marks.push(item.mark)
            })

            layer.open({
                skin: 'user-info-layer',
                btn: ['关闭']
